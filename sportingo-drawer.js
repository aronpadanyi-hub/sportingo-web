...
(function() {
  ...
  async function loadErtekelesek() {
    ...
    // ── meglevoList rendezése: legfrissebb (updated_at, fallback: letrehozas_datum) felül ──
    meglevoList.sort(function(a, b) {
      var aDate = a.review && (a.review.updated_at || a.review.letrehozas_datum);
      var bDate = b.review && (b.review.updated_at || b.review.letrehozas_datum);
  ...
  function spMountAuthHints() {
    // Fut minden openLoginModal() után – ekkor már a DOM-ban vannak a .sp-google-btn elemek
    try {
      ...
      document.querySelectorAll('.sp-google-btn').forEach(function(btn) {
        // Duplikáció-védelem: csak egyszer adjuk hozzá
      ...
      // Timer wrapper: Google kattintás → 3.5 mp után hint megjelenik
      document.querySelectorAll('.sp-google-btn').forEach(function(btn) {
        if (btn._spHintBound) return;
        btn._spHintBound = true;
  ...
  function openLoginModal() {
    ...
    setLoginView('login');
    // Hint mount: DOM már kész, most kerül be a hint elem a gombok alá
    try { spMountAuthHints(); } catch(e) {}
    // Hint alaphelyzetbe állítása minden megnyitáskor
    try { spHideAuthHint(); clearTimeout(_spGoogleHintTimer); } catch(e) {}
  ...
  window.spGoogleLogin = async function() {
    ...
    if (error) {
      ...
      // Hint azonnal megjelenik auth error esetén
      try { clearTimeout(_spGoogleHintTimer); spShowAuthHint(); } catch(e) {}
    }
  };
